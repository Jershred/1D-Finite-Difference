...
## -*- texinfo -*- 
## @deftypefn {} {@var{retval} =} erreur_spatiale_cn (@var{input1}, @var{input2})
##
## @seealso{}
## @end deftypefn
...
function retval = erreur_spatiale_cn (input1, input2)
clear all
clc
...
%Erreur de la methode C-N pour chaque serie de point dans N
Err_cn=zeros(1,length(N));

for i=1:4 
  %Chaque iteration correspond a un nombre different 
  ...
  Ccn(Nx-1,1)=r*y; %prise en compte de la condition de Dirichlet

  A1cn=diag(ones(Nx-1,1).*(1+r),0);
  A2cn=diag(ones(Nx-2,1).*(-r/2),-1);
  A3cn=diag(ones(Nx-2,1).*(-r/2),1);
  Acn=A1cn+A2cn+A3cn;
  Acn(1,2)=-r; %Prise en compte de la condition de Neumann

  B1cn=diag(ones(Nx-1,1).*(1-r),0);
  B2cn=diag(ones(Nx-2,1).*(r/2),-1);
  B3cn=diag(ones(Nx-2,1).*(r/2),1);
  Bcn=B1cn+B2cn+B3cn;
  Bcn(1,2)=r;%Prise en compte de la condition de Neumann

  Ainvcn=inv(Acn);

  Tcn=zeros(Nx-1,length(t));
  Tcn(:,1)=T0cn(1:Nx-1);
  ...
  %Calcul de l'erreur de la methode de C-N pour le dx de l'iteration en cours
  Err_cn(1,i)=abs((Texacte(end-1,end)-Tcn(end,end))/Texacte(end-1,end));
...
Affine_Err_cn=polyfit(logdx,logErr_cn,1)

hold on
plot(logdx,logErr_cn,'kx')
plot(logdx,polyval(Affine_Err_cn,logdx),'g')
legend('Crank-Nicholson','Approximation affine')
title('Schema de Crank-Nicholson')
hold off
